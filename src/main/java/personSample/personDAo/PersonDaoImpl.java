...
//            statement.executeUpdate(query);
    ...
    public boolean addPerson(Person person) {
        ...
        String add1 = String.format(" insert into person(person_id, first_name, last_name, birth_date) values (%d, '%s', '%s', '%s')",
                person.getPersonID(), person.getFirstName(),person.getLastName(), person.getBirthDate().format(formatter));
        int result = 0;
        try {
            Statement statement = connection.createStatement();
//
            result = statement.executeUpdate(add1);
            statement.close();
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
